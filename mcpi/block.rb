class Block
    ...
    # initialize class with id and optional data
    def initialize(id, data=0)
        @id = id
        @data = data
    end

    def withData(data)
        return Block.new(@id, data)
    end
    ...
    def ==(blockToCompare)
        return ([@id, @data].hash == [blockToCompare.id, blockToCompare.data].hash)
    end
